...
version=$(journalctl -u $folder.service --no-hostname -o cat | grep "MOONVEIL L2 Muse Node" | awk '{print $6}')
nft=$(journalctl -u $folder.service --no-hostname -o cat | grep "successfully delegated" | awk '{print $7}' | sed 's/#//')
